...
def plotJSONDataVsObs(obs_data, out_folder, best_noise_lum=0, best_noise_lag=0):
    ...
    for const_json_name in json_files:
        ...
        try:
            ...
            simulation_manual_MetSim_object = runSimulation_dynesty(guess_manual, obs_data, var_names_manual, fixed_values_manual)
            if best_noise_lum!=0 and best_noise_lag!=0:
...
def _worker_simulate_and_interp(sample_equal_row):
    ...
    try:
        ...
        sim  = runSimulation_dynesty(pars, obs_data, variables, fixed_values)
...
def posteriorBandsVsHeightParallel(
    ...
    sim_best  = runSimulation_dynesty(best_pars, obs_data, variables, fixed_values)
...
def plotDynestyResults(dynesty_run_results, obs_data, flags_dict, fixed_values, output_folder='', file_name='', log_file='', cores=None, save_backup=False):
    ...
    best_guess_obj_plot = runSimulation_dynesty(best_guess, obs_data, variables, fixed_values)
    ...
    approx_mode_obj_plot = runSimulation_dynesty(approx_modes_all, obs_data, variables, fixed_values)
    ...
    mean_obj_plot = runSimulation_dynesty(all_samples_mean, obs_data, variables, fixed_values)
    ...
    median_obj_plot = runSimulation_dynesty(all_samples_median, obs_data, variables, fixed_values)
...
def runSimulation_dynesty(parameter_guess, real_event, var_names, fix_var):
...
def logLikelihoodDynesty(guess_var, obs_metsim_obj, flags_dict, fix_var, timeout=20):
    ...
    if os.name != 'posix':
        ...
        simulation_results = runSimulation_dynesty(guess_var, obs_metsim_obj, var_names, fix_var)
    else:
        # Set timeout handler
