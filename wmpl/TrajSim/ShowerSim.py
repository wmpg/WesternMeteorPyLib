...
class AblationModelVelocity(object):
    ...
    def getSimulation(self, v_init, zangle, beg_height):
        ...
        # Run the simulation with the given parameters (run single body)
        frag_list, _ = runSimulation(self.met, self.consts)

        # Get the results for single-body modelling
        sim_results = np.array(frag_list[0].results_list)
        ...
        if len(time) == 0:
            print('Meteor not ablation at all, skipping it...')
            return False
...
def simulateMeteorShower(station_list, meteor_velocity_models, n_meteors, met_shower_model, state_vect_model,\
    ...
    for sample in met_shower_model.generate():
        ...
        for stat in station_list:
            ...
            if np.degrees(elev) < 5:
                print('Radiant below the local horizon!')
                skip_meteor = True
                break
        ...
        # Skip trajectories with meteors going up
        if np.degrees(sim_meteor.orbit.zc) > 80:
            print('Meteor propagating up!')
            continue
...
if __name__ == "__main__":
    ...
    # ### SIMULATED 2 STATION MODERATE STATION PARAMETERS ###
    # ##########################################################################################################

    # system_name = 'CAMSsim_2station'
    ...
    # # Cameras FOV widths (degrees)
    # fov_widths = [64.0, 64.0]

    # # Cameras FOV heights (degrees)
    # fov_heights = [48.0, 48.0]

    # # Limiting magnitudes (needed only for ablation simulation)
    # lim_magnitudes = [5.0, 5.0]

    # # Powers of zero-magnitude meteors (Watts) (needed only for ablation simulation)
    # P_0m_list = [1210, 1210]

    # # Minimum angular velocity for detection (deg/s)
    # min_ang_velocities = [2.0, 2.0]
    ...
    # # Solar longitude of peak activity in degrees
    # sol_max = 126.5
    # sol_slope = 0.08 # Fitted on CMOR data
    ...
    # # Mass index
    # mass_index = 1.9 # Pokorny 2016 mass index paper, read from the graph for 2012

    # # # Mass range (log of mass in kg) seen by the system (allsky, 40 km/s, SDA)
    # # mass_min = -3.5
    # # mass_max = -0.7

    # # # Mass range (log of mass in kg) seen by the system (CAMS, 40 km/s, SDA)
    # # mass_min = -5.9
    # # mass_max = -3.2

    # # Mass range (log of mass in kg) seen by the system (CAMO, 40 km/s, SDA)
    # mass_min = -6.7
    # mass_max = -4.15
