...
from wmpl.Utils.Math import findClosestPoints, lineFunc, meanAngle, mergeClosePoints, vectMag
from wmpl.Utils.Physics import calcMass, calcRadiatedEnergy, dynamicPressure
...
class SimulationResults(object):
    def __init__(self, const, frag_main, results_list, wake_results):
        ...
        (
            self.time_arr,
            self.luminosity_arr,
            self.luminosity_main_arr,
            self.luminosity_eroded_arr,
            self.electron_density_total_arr,
            self.tau_total_arr,
            self.tau_main_arr,
            self.tau_eroded_arr,
            self.brightest_height_arr,
            self.brightest_length_arr,
            self.brightest_vel_arr,
            self.leading_frag_height_arr,
            self.leading_frag_length_arr,
            self.leading_frag_vel_arr,
            self.mass_total_active_arr,
            self.main_mass_arr,
            self.main_height_arr,
            self.main_length_arr,
            self.main_vel_arr,
        ) = results_list.T
        ...
        with warnings.catch_warnings():
            ...
            self.abs_magnitude_eroded[np.isnan(self.abs_magnitude_eroded)] = np.nanmax(
                self.abs_magnitude_eroded
            )
        ...
        if const.fragmentation_show_individual_lcs:
            for frag_entry in const.fragmentation_entries:
                ...
                if len(frag_entry.main_time_data):
                    ...
                    frag_entry.main_height_data = leading_frag_ht_interpol(
                        np.array(frag_entry.main_time_data)
                    )

                    # Compute the magnitude
                    frag_entry.main_abs_mag = -2.5 * np.log10(
                        np.array(frag_entry.main_luminosity) / const.P_0m
                    )

                    # Compute the luminosity weigthed tau
                    frag_entry.main_tau = np.array(frag_entry.main_tau_over_lum) / np.array(
                        frag_entry.main_luminosity
                    )
                ...
                if len(frag_entry.grains_time_data):
                    ...
                    # Compute the magnitude
                    frag_entry.grains_abs_mag = -2.5 * np.log10(
                        np.array(frag_entry.grains_luminosity) / const.P_0m
                    )

                    # Compute the luminosity weigthed tau
                    frag_entry.grains_tau = np.array(frag_entry.grains_tau_over_lum) / np.array(
                        frag_entry.grains_luminosity
                    )
        ...
        if np.any(wake_results):
            ...
            self.wake_max_lum = max(
                [max(wake.wake_luminosity_profile) for wake in wake_results if wake is not None]
            )
    ...
    def __init__(self, met, traj):
        ...
        if self.met is not None:
            ...
            for site in met.sites:
                ...
                ra_picks, dec_picks = altAz2RADec_vect(
                    np.pi / 2 - phi_picks, np.pi / 2 - theta_picks, jd_picks, met.lat[site], met.lon[site]
                )
                ...
                for i, (jd, ra, dec, mag) in enumerate(np.c_[jd_picks, ra_picks, dec_picks, mag_picks]):
                    ...
                    obs_cpa, rad_cpa, d = findClosestPoints(
                        stat, meas, self.traj.state_vect_mini, self.traj.radiant_eci_mini
                    )
                    ...
                    state_vect_dist.append(
                        state_vect_dist_sign * vectMag(self.traj.state_vect_mini - rad_cpa)
                    )
            ...
            for line in f:
                ...
                if line.startswith(station_label):
                    ...
                    if current_station is not None:
                        ...
                        print(
                            "Loaded {:d} mag points from station: {:s}".format(
                                len(time_data), current_station
                            )
                        )
...
"""
        ...
        initial_entry = FragmentationEntry(
            frag_type="# INIT",
        ...
        if self.gui.simulation_results is not None:
            ...
            final_fragment = FragmentationEntry(
                frag_type="# END",
...
def extractConstantParams(const_original, params, param_string, mini_norm_handle):
    ...
    if 'e' in param_string:
        (
            const.erosion_height_start,
            const.erosion_coeff,
            const.erosion_height_change,
            const.erosion_coeff_change,
            const.erosion_mass_index,
            const.erosion_mass_min,
            const.erosion_mass_max,
        ) = params[param_index : param_index + 7]
    ...
    if 'd' in param_string:
        (
            ...
            const.disruption_erosion_coeff,
            const.disruption_mass_index,
            const.disruption_mass_min_ratio,
            const.disruption_mass_max_ratio,
            const.disruption_mass_grain_ratio,
        ) = params[param_index : param_index + 6]
...
def fitResiduals(
    ...
    len_sim_interpol = scipy.interpolate.interp1d(
        sr.brightest_height_arr, sr.brightest_length_arr, bounds_error=False, fill_value=0
    )
    mag_sim_interpol = scipy.interpolate.interp1d(
        sr.brightest_height_arr, sr.abs_magnitude, bounds_error=False, fill_value=10
    )
    time_sim_interpol = scipy.interpolate.interp1d(
        sr.brightest_height_arr, sr.time_arr, bounds_error=False, fill_value=0
    )
    ...
    if 'fragmentation_entries' in const_json:
        ...
        if len(const_json['fragmentation_entries']) > 0:
            for frag_entry_dict in const_json['fragmentation_entries']:
                ...
                frag_entry_dict = {
                    key: frag_entry_dict[key]
                    for key in frag_entry_dict
...
class MetSimGUI(QMainWindow):
    def __init__(self, traj_path, const_json_file=None, met_path=None, lc_path=None, wid_files=None):
        ...
        if const_json_file is not None:
            ...
            if len(self.const.fragmentation_entries):
                ...
                self.fragmentation = FragmentationContainer(
                    self, os.path.join(self.dir_path, self.const.fragmentation_file_name)
                )
        ...
        print(
            "Atmospheric mass density fit for the range of heights: {:.2f} - {:.2f} km".format(
        ...
        self.checkBoxFragmentationShowIndividualLCs.clicked.connect(
            self.checkBoxFragmentationShowIndividualLCsSignal
        )
    ...
    def loadWakeFile(self, file_path):
        ...
        with open(file_path) as f:
            for line in f:
                ...
                # Calculate RA/Dec
                ra, dec = altAz2RADec(
                    np.pi / 2 - np.radians(phi),
                    np.pi / 2 - np.radians(th),
                    self.traj.jdt_ref,
                    lat_dict[site_id],
                    lon_dict[site_id],
                )
                ...
                # Compute the station coordinates at the given time
                stat = geo2Cartesian(
                    lat_dict[site_id], lon_dict[site_id], ele_dict[site_id], self.traj.jdt_ref
                )
    ...
    def calcPhotometricMass(self):
        ...
        # If the magnitudes are given
            ...
            for obs in self.traj.observations:
                ...
                for t, mag in zip(
                    obs.time_data[obs.ignore_list == 0], obs.absolute_magnitudes[obs.ignore_list == 0]
                ):
        ...
        photom_mass = calcMass(
            np.array(time_arr),
            np.array(mag_arr),
            self.traj.orbit.v_avg,
            tau=self.const.lum_eff / 100,
    ...
    def updateGrainDiameters(self):
        ...
        grain_diam_min = 2 * (3 / (4.0 * np.pi) * self.const.erosion_mass_min / self.const.rho_grain) ** (
            1 / 3.0
        )

        # Update diameter label
        self.erosionMinMassDiameter.setText(
            """d<span style="vertical-align:sub;">min</span> = """ + "{:.0f}".format(1e6 * grain_diam_min)
        )

        # Compute maximum grain diameter (m)
        grain_diam_max = 2 * (3 / (4.0 * np.pi) * self.const.erosion_mass_max / self.const.rho_grain) ** (
            1 / 3.0
        )

        # Update diameter label
        self.erosionMaxMassDiameter.setText(
            """d<span style="vertical-align:sub;">max</span> = """ + "{:.0f}".format(1e6 * grain_diam_max)
        )
    ...
    def checkBoxErosionRhoSignal(self, event):
        ...
        self.inputErosionRhoChange.setDisabled(
            (not self.erosion_different_rho) or (not self.const.erosion_on)
        )
    ...
    def checkBoxErosionAblationCoeffSignal(self, event):
        ...
        self.inputErosionAblationCoeffChange.setDisabled(
            (not self.erosion_different_sigma) or (not self.const.erosion_on)
        )
    ...
    def checkBoxDisruptionErosionCoeffSignal(self, event):
        ...
        self.inputDisruptionErosionCoeff.setDisabled(
            (not self.disruption_different_erosion_coeff) or (not self.const.disruption_on)
        )
    ...
    def readInputBoxes(self):
        ...
        self.const.zenith_angle = np.radians(
            self._tryReadBox(self.inputZenithAngle, np.degrees(self.const.zenith_angle))
        )
        ...
        self.const.erosion_height_start = 1000 * self._tryReadBox(
            self.inputErosionHtStart, self.const.erosion_height_start / 1000
        )
        self.const.erosion_bins_per_10mass = int(
            self._tryReadBox(self.inputBinsPer10m, self.const.erosion_bins_per_10mass)
        )
        self.const.erosion_coeff = (
            self._tryReadBox(self.inputErosionCoeff, self.const.erosion_coeff * 1e6) / 1e6
        )
        self.const.erosion_height_change = 1000 * self._tryReadBox(
            self.inputErosionHtChange, self.const.erosion_height_change / 1000
        )
        self.const.erosion_coeff_change = (
            self._tryReadBox(self.inputErosionCoeffChange, self.const.erosion_coeff_change * 1e6) / 1e6
        )
        self.const.erosion_mass_index = self._tryReadBox(
            self.inputErosionMassIndex, self.const.erosion_mass_index
        )
        self.const.erosion_mass_min = self._tryReadBox(self.inputErosionMassMin, self.const.erosion_mass_min)
        self.const.erosion_mass_max = self._tryReadBox(self.inputErosionMassMax, self.const.erosion_mass_max)
        ...
        if self.erosion_different_rho:
            self.const.erosion_rho_change = self._tryReadBox(
                self.inputErosionRhoChange, self.const.erosion_rho_change
            )
        else:
        ...
        if self.erosion_different_sigma:
            self.const.erosion_sigma_change = (
                self._tryReadBox(self.inputErosionAblationCoeffChange, self.const.erosion_sigma_change * 1e6)
                / 1e6
            )
        else:
            # Otherwise, use the same bulk density value
        ...
        self.const.compressive_strength = 1000 * self._tryReadBox(
            self.inputCompressiveStrength, self.const.compressive_strength / 1000
        )
        ...
        if self.disruption_different_erosion_coeff:
            self.const.disruption_erosion_coeff = (
                self._tryReadBox(self.inputDisruptionErosionCoeff, self.const.disruption_erosion_coeff * 1e6)
                / 1e6
            )
        else:
        ...
        self.const.disruption_mass_grain_ratio = (
            self._tryReadBox(self.inputDisruptionMassGrainRatio, self.const.disruption_mass_grain_ratio * 100)
            / 100
        )
        self.const.disruption_mass_index = self._tryReadBox(
            self.inputDisruptionMassIndex, self.const.disruption_mass_index
        )
        self.const.disruption_mass_min_ratio = (
            self._tryReadBox(self.inputDisruptionMassMinRatio, self.const.disruption_mass_min_ratio * 100)
            / 100
        )
        self.const.disruption_mass_max_ratio = (
            self._tryReadBox(self.inputDisruptionMassMaxRatio, self.const.disruption_mass_max_ratio * 100)
            / 100
        )
        ...
        self.pso_iterations = self._tryReadBox(
            self.inputAutoFitPSOIterations, self.pso_iterations, value_type=int
        )
        self.pso_particles = self._tryReadBox(
            self.inputAutoFitPSOParticles, self.pso_particles, value_type=int
        )

        self.autofit_mag_weight = self._tryReadBox(self.inputAutoFitMagWeight, self.autofit_mag_weight)
        self.autofit_lag_weight_ht_change = 1000 * self._tryReadBox(
            self.inputAutoFitLagWeightHtChange, self.autofit_lag_weight_ht_change / 1000
        )
        self.autofit_lag_weights = self._tryReadBox(
            self.inputAutoFitLagWeights,
            self.autofit_lag_weights,
        ...
        # Height of electron line density measurement
        self.const.electron_density_meas_ht = 1000 * self._tryReadBox(
            self.inputElectronDensityHt, self.const.electron_density_meas_ht / 1000
        )

        # Electron line density measurement
        self.const.electron_density_meas_q = self._tryReadBox(
            self.inputElectronDensity, self.const.electron_density_meas_q
        )
    ...
    def updateCommonPlotFeatures(self, plt_handle, sr, plot_text=False):
        ...
        if (
            self.const.erosion_on
            and (self.const.erosion_height_start / 1000 >= y_min)
            ...
            plt_handle.plot(
                x_arr,
                np.zeros_like(x_arr) + self.const.erosion_height_start / 1000,
                linestyle='dashed',
                color='k',
                alpha=0.25,
            )
            ...
            if plot_text:
                plt_handle.text(
                    x_min,
                    TEXT_LABEL_HT_PAD + self.const.erosion_height_start / 1000,
                    "Erosion beg",
                    size=7,
                    alpha=0.5,
                )
        ...
        if (
            (self.const.erosion_height_change > self.traj.rend_ele)
            and (self.const.erosion_height_change / 1000 >= y_min)
            and (self.const.erosion_height_change / 1000 <= y_max)
        ):

            # Plot a line marking erosion change
            plt_handle.plot(
                x_arr,
                np.zeros_like(x_arr) + self.const.erosion_height_change / 1000,
                linestyle='dashed',
                color='k',
                alpha=0.25,
            )
            ...
            if plot_text:
                plt_handle.text(
                    x_min,
                    TEXT_LABEL_HT_PAD + self.const.erosion_height_change / 1000,
                    "Erosion change",
                    size=7,
                    alpha=0.5,
                )
        ...
        if self.const.disruption_on and (self.const.disruption_height is not None):
            ...
            if (self.const.disruption_height / 1000 >= y_min) and (
                self.const.disruption_height / 1000 <= y_max
            ):

                plt_handle.plot(
                    x_arr,
                    np.zeros_like(x_arr) + self.const.disruption_height / 1000,
                    linestyle='dotted',
                    color='k',
                    alpha=0.5,
                )
                ...
                if plot_text:
                    plt_handle.text(
                        x_min,
                        TEXT_LABEL_HT_PAD + self.const.disruption_height / 1000,
                        "Disruption",
                        size=7,
                        alpha=0.5,
                    )
    ...
    def updateMagnitudePlot(self, show_previous=False):
        ...
        if self.lc_data is None:
            for obs in self.traj.observations:
                ...
                self.magnitudePlot.canvas.axes.plot(
                    abs_mag_data,
                    height_data,
                    marker='x',
                    linestyle='dashed',
        ...
        if sr is not None:
            ...
            sim_radiated_energy = calcRadiatedEnergy(sr.time_arr, sr.abs_magnitude, P_0m=self.const.P_0m)
        ...
        if sr is not None:
            ...
            self.magnitudePlot.canvas.axes.plot(
                sr.abs_magnitude, sr.leading_frag_height_arr / 1000, label='Simulated', color='k', alpha=0.5
            )

            if self.electron_density_plot_show:

                # Plot the simulated electron line density
                q_sim_plot = self.electronDensityPlot.plot(
                    np.log10(sr.electron_density_total_arr),
                    sr.leading_frag_height_arr / 1000,
                    color='b',
                    alpha=0.5,
                    linestyle='dashed',
                )
            ...
            if self.const.fragmentation_show_individual_lcs:
                ...
                for frag_entry in self.const.fragmentation_entries:
                    ...
                    if len(frag_entry.main_height_data):
                        ...
                        self.magnitudePlot.canvas.axes.plot(
                            frag_entry.main_abs_mag,
                            frag_entry.main_height_data / 1000,
                            color=line_color,
                            linestyle='dashed',
                            linewidth=1,
                            alpha=0.5,
                    ...
                    if len(frag_entry.grains_height_data):
                        ...
                        self.magnitudePlot.canvas.axes.plot(
                            frag_entry.grains_abs_mag,
                            frag_entry.grains_height_data / 1000,
                            color=line_color,
                            linestyle='dashed',
                            linewidth=1,
                            alpha=0.5,
        ...
        self.magnitudePlot.canvas.axes.set_ylim(
            [self.plot_end_ht + END_HT_PAD, self.plot_beg_ht + BEG_HT_PAD]
        )
        ...
        self.magnitudePlot.canvas.axes.legend(
            handles, labels, prop={'size': LEGEND_TEXT_SIZE}, loc='upper right'
        )
    ...
    def updateVelocityPlot(self, show_previous=False):
        ...
        for obs in self.traj.observations:
            ...
            self.velocityPlot.canvas.axes.plot(
                vel_data, height_data, marker='o', label=obs.station_id, markersize=1, linestyle='none'
            )
        ...
        self.velocityPlot.canvas.axes.plot(
            self.traj.orbit.v_init / 1000,
            self.traj.rbeg_ele / 1000,
            marker='x',
            label="Vinit obs",
            markersize=5,
            linestyle='none',
        ...
        self.velocityPlot.canvas.axes.plot(
            np.zeros_like(avg_vel_ht_plot_arr) + self.traj.orbit.v_avg / 1000,
            avg_vel_ht_plot_arr,
            label="Vavg obs",
            linestyle='dashed',
            color='k',
        ...
        if sr is not None:
            ...
            self.velocityPlot.canvas.axes.plot(
                sr.brightest_vel_arr / 1000,
                sr.brightest_height_arr / 1000,
                label='Simulated - brightest',
                color='k',
                alpha=0.5,
            )
            ...
            # Plot the simulated velocity at the brightest point
            self.velocityPlot.canvas.axes.plot(
                sr.leading_frag_vel_arr / 1000,
                sr.leading_frag_height_arr / 1000,
                label='Simulated - leading',
                color='k',
                alpha=0.5,
                linestyle="dashed",
            )
            ...
            sim_vel_obs_range = sr.brightest_vel_arr[
                (sr.brightest_height_arr <= self.traj.rbeg_ele)
            ...
            self.velocityPlot.canvas.axes.plot(
                np.zeros_like(avg_vel_ht_plot_arr) + v_avg_sim / 1000,
                avg_vel_ht_plot_arr,
                label="Vavg sim",
                linestyle='dotted',
                color='k',
    ...
    def updateLagPlot(self, show_previous=False):
        ...
        if sr is not None:
            ...
            if len(brightest_len_arr):
                ...
                brightest_lag_sim = (
                    brightest_len_arr
                    - brightest_len_arr[0]
                    - self.traj.orbit.v_init
                    * np.arange(0, self.const.dt * len(brightest_len_arr), self.const.dt)[
                        : len(brightest_len_arr)
                    ]
                )
                ...
                temp_arr = temp_arr[
                    (sr.leading_frag_height_arr <= self.traj.rbeg_ele)
                ...
                leading_lag_sim = (
                    ...
                    - self.traj.orbit.v_init
                    * np.arange(0, self.const.dt * len(leading_frag_len_arr), self.const.dt)[
                        : len(leading_frag_len_arr)
                    ]
                )
        ...
        lag_residuals_plot = self.lagPlot.canvas.figure.add_subplot(
            1, 2, 2, label='lag residuals', sharey=lag_plot
        )
        ...
        for obs in self.traj.observations:
            ...
            # Plot observed lag
            lag_handle = lag_plot.plot(
                obs.lag[obs.ignore_list == 0],
                height_data / 1000,
                marker='x',
                linestyle='dashed',
                label=obs.station_id,
            ...
            if sr is not None:
                ...
                brightest_residuals = obs.lag[obs.ignore_list == 0][obs_height_indices] - brightest_interp(
                    -obs_hts
                )

                # Plot the lag residuals
                lag_residuals_plot.scatter(
                    brightest_residuals,
                    obs_hts / 1000,
                    marker='+',
                    c=lag_handle[0].get_color(),
                    label="Brightest, {:s}".format(obs.station_id),
                    s=6,
                )
                ...
                # Get simulated lags at the same height as observed
                leading_interp = scipy.interpolate.interp1d(
                    -leading_ht_arr, leading_lag_sim, bounds_error=False, fill_value=0
                )
                obs_height_indices = height_data > np.min(leading_ht_arr)
                obs_hts = height_data[obs_height_indices]
                leading_residuals = obs.lag[obs.ignore_list == 0][obs_height_indices] - leading_interp(
                    -obs_hts
                )
                ...
                lag_residuals_plot.scatter(
                    ...
                    marker='s',
                    c=lag_handle[0].get_color(),
                    label="Leading, {:s}".format(obs.station_id),
                    s=6,
                )
    ...
    def redrawWakeHeightLine(self, plt_handle, line_handles, label_handles=None, draw_label=False):
        ...
        for i, ax in enumerate(plt_handle.canvas.figure.get_axes()):
            ...
            line_handle = ax.plot(
                x_arr,
                np.zeros_like(x_arr) + self.wake_plot_ht / 1000,
                linestyle='dashed',
                color='green',
                alpha=0.5,
            )[0]
            ...
            if draw_label:
                ...
                label_handle = ax.text(
                    x_min,
                    TEXT_LABEL_HT_PAD + self.wake_plot_ht / 1000,
                    "Wake plot",
                    size=7,
                    color='green',
                    alpha=0.5,
    ...
    def normalizeObservedWake(
        self,
        len_array,
        wake_intensity_array,
        wake_sim,
        simulated_peak_luminosity,
        simulated_integrated_luminosity,
        simulated_peak_length,
        ...
        if wake_sim is not None:
            ...
            # Align the wake by cross correlation
                ...
                sim_wake_interp = scipy.interpolate.interp1d(
                    wake_sim.length_array, wake_sim.wake_luminosity_profile, bounds_error=False, fill_value=0
                )
                model_wake_obs_len_sample = sim_wake_interp(-len_array)

                # Correlate the wakes and find the shift
                wake_shift = (
                    np.argmax(np.correlate(model_wake_obs_len_sample, wake_intensity_array, "full")) + 1
                )
    ...
    def updateWakePlot(self, show_previous=False):
        ...
        self.magnitudePlotWakeLines, self.magnitudePlotWakeLineLabels = self.redrawWakeHeightLine(
            self.magnitudePlot,
            self.magnitudePlotWakeLines,
            label_handles=self.magnitudePlotWakeLineLabels,
            draw_label=True,
        )
        self.velocityPlotWakeLines, _ = self.redrawWakeHeightLine(
            self.velocityPlot, self.velocityPlotWakeLines
        )
        ...
        if self.wake_heights is not None:
            ...
            self.wake_ht_current_index = np.argmin(
                np.abs(np.array([w[0] for w in self.wake_heights]) - self.wake_plot_ht)
            )
        ...
        if sr is not None:
            ...
            if wake is not None:
                ...
                wake_ht_plot.plot(
                    wake.length_array, wake.wake_luminosity_profile, label='Simulated', color='k', alpha=0.5
                )
                ...
                if self.wake_show_mass_bins:
                    ...
                    if frag_main is not None:
                        ...
                        #     s=10, alpha=0.5)
                        mass_ax.scatter(
                            frag_main.length - wake.leading_frag_length, frag_main.m, c='r', s=20, alpha=0.5
                        )
        ...
        if self.current_wake_container is not None:
            ...
            len_array, wake_intensity_array = self.normalizeObservedWake(
                len_array,
                wake_intensity_array,
                wake,
                simulated_peak_luminosity,
        ...
        for i, plot_ht in enumerate(sorted(wake_plotting_heights)):
            ...
            if wake is not None:
                ...
                luminosity_profile_scaled = plot_shift + wake.wake_luminosity_profile / np.max(
                    wake.wake_luminosity_profile
                )

                # Plot the simulated wake and scale it to the maximum lumino
                wake_overview_plot.plot(
                    wake.length_array,
                    luminosity_profile_scaled,
                    label='Simulated',
                    color='k',
                    alpha=0.5,
                    linewidth=1,
                ...
                wake_overview_plot.text(
                    wake.length_array[0],
                    luminosity_profile_scaled[0] - 0.1 / n_plots,
                    "{:.2f} km".format(plot_ht / 1000),
                    ha='center',
                    va='top',
                    size=6,
                )
            ...
            if self.wake_heights is not None:
                ...
                # Find the index of the observed wake that's closest to the given plot height
                wake_ht_index = np.argmin(np.abs(np.array([w[0] for w in self.wake_heights]) - plot_ht))
                ...
                # Plot the observed wake
                wake_overview_plot.plot(
                    -len_array, wake_intensity_array_scaled, color='k', linestyle='dotted', linewidth=1
                )

                # If the simulations are not shown, plot the wake heights
                if wake is None:
                    wake_overview_plot.text(
                        -self.const.wake_extension,
                        wake_intensity_array_scaled[-1] - 0.1 / n_plots,
                        "{:.2f} km".format(plot_ht / 1000),
                        ha='center',
                        va='top',
                        size=6,
    ...
    def plotAtmDensity(self):
        ...
        atm_densities = np.array(
            [getAtmDensity(lat_mean, lon_mean, ht, self.traj.jdt_ref) for ht in height_arr]
        )
        ...
        self.mpw.canvas.axes.semilogx(atm_densities, height_arr / 1000, label="NRLMSISE-00", color='k')

        # Poly poly fit
        self.mpw.canvas.axes.semilogx(
            atm_densities_poly, height_arr / 1000, label="Polynomial fit", color='red', linestyle="dashed"
        )
    ...
    def plotDynamicPressure(self):
        ...
        if self.simulation_results is not None:
            ...
            brightest_dyn_pressure = dynamicPressure(
                lat_mean,
                lon_mean,
                sr.brightest_height_arr,
                self.traj.jdt_ref,
                sr.brightest_vel_arr,
                gamma=self.const.gamma,
            )
            leading_frag_dyn_pressure = dynamicPressure(
                lat_mean,
                lon_mean,
                sr.leading_frag_height_arr,
                self.traj.jdt_ref,
                sr.leading_frag_vel_arr,
                gamma=self.const.gamma,
            )

            # Plot dyn pressure
            self.mpw.canvas.axes.plot(
                brightest_dyn_pressure / 1e6,
                sr.brightest_height_arr / 1000,
                label="Brightest",
                color='k',
                alpha=0.5,
            ...
            self.mpw.canvas.axes.scatter(
                peak_dyn_pressure,
                peak_dyn_pressure_ht,
    ...
    def plotMassLoss(self):
        ...
        if self.simulation_results is not None:
            ...
            main_dyn_pressure = dynamicPressure(
                lat_mean,
                lon_mean,
                sr.main_height_arr,
                self.traj.jdt_ref,
            ...
            self.mpw.canvas.axes.loglog(
                brightest_dyn_pressure[mass_filter] / 1e6,
                sr.mass_total_active_arr[mass_filter],
                label="Total mass",
                color='k',
                linewidth=1,
                linestyle='dashed',
            ...
            self.mpw.canvas.axes.loglog(
                main_dyn_pressure[mass_filter] / 1e6,
                sr.main_mass_arr[mass_filter],
                label="Main mass",
                color='k',
                linewidth=2,
            )
    ...
    def plotLumEfficiency(self):
        ...
        self.mpw.canvas.axes.plot(
            100 * sr.tau_total_arr, sr.leading_frag_height_arr / 1000, color='k', alpha=0.5, label="Total"
        )
    ...
    def runSimulationGUI(self):
        ...
        if self.const.fragmentation_on and (self.fragmentation is None):
            frag_error_message = QMessageBox(
                QMessageBox.Critical,
                "Fragmentation file error",
                "Fragmentation is enabled but no fragmentation file is set.",
            )
            frag_error_message.setInformativeText(
                "Either load an existing fragmentation file or create a new one."
            )
    ...
    def autoFit(self):
        ...
        meteoroid_bounds = [
            [0.1 * const.m_init, 2 * const.m_init],
        ...
        if const.disruption_on:
            ...
            disruption_bounds = [
                [0.1 * const.compressive_strength, 10 * const.compressive_strength],
                [0.0, 1.0 / 1e6],
                [1.0, 3.0],
                [0.01, 50.0],
                [0.1, 80.0],
        ...
        print(
            "Starting residual value: {:.5f}".format(
                fitResiduals(
                    p0_normed,
                    fit_input_data,
                    param_string,
                    ...
                    self.traj,
                    mini_norm_handle,
                    mag_weight=self.autofit_mag_weight,
                    lag_weights=self.autofit_lag_weights,
                    lag_weight_ht_change=self.autofit_lag_weight_ht_change,
                    verbose=False,
                )
            )
        )
        ...
        if self.autofit_method == "Local":
            ...
            res = scipy.optimize.minimize(
                ...
                args=(
                    ...
                    self.traj,
                    mini_norm_handle,
                    self.autofit_mag_weight,
                    self.autofit_lag_weights,
                    self.autofit_lag_weight_ht_change,
                    True,
                    self,
                ),
        ...
        # PSO optimization
            ...
            if self.autofit_method == "PSO local":
                ...
                init_pos = np.random.normal(
                    loc=p0_normed,
                    scale=0.2 + np.zeros_like(p0_normed),
            ...
            cost, pos = optimizer.optimize(
                fitResidualsListArguments,
                iters=self.pso_iterations,
                n_processes=mp.cpu_count() - 1,
                fit_input_data=fit_input_data,
                param_string=param_string,
                const_original=const,
                traj=self.traj,
                mini_norm_handle=mini_norm_handle,
                mag_weight=self.autofit_mag_weight,
                lag_weights=self.autofit_lag_weights,
                lag_weight_ht_change=self.autofit_lag_weight_ht_change,
                verbose=False,
    ...
    def newFragmentationFile(self):
        ...
        fragmentation_file_path = QFileDialog.getSaveFileName(
            self,
            "Choose the fragmentation file",
            os.path.join(self.dir_path, "metsim_fragmentation.txt"),
            "Fragmentation file (*.txt)",
        )[0]
    ...
    def loadFragmentationFile(self):
        ...
        fragmentation_file_path = QFileDialog.getOpenFileName(
            self, "Choose the fragmentation file", self.dir_path, "Fragmentation file (*.txt)"
        )[0]
    ...
    def saveUpdatedOrbit(self):
        ...
        orb = calcOrbit(
            ...
            self.traj.v_init + v_init_diff,
            self.traj.v_avg + v_init_diff,
            self.traj.state_vect_mini,
            self.traj.rbeg_jd,
        )
        ...
        traj_updated.saveReport(
            dir_path, report_file_name, uncertainties=self.traj.uncertainties, verbose=False
        )
    ...
    def saveVideo(self, event):
        ...
        # Init the Gaussian
        gauss = scipy.stats.multivariate_normal(
            [0.0, 0.0],
            [
                [self.const.wake_psf / np.sqrt(2) / plate_scale, 0],
        ...
        for i, (wake, ht) in enumerate(
            zip(
                self.simulation_results.wake_results[wake_beg_indx:wake_end_indx],
            ...
            for px, lum in zip(pixel_length, luminosities):
                ...
                gauss_eval = (
                    ...
                    / plate_scale
                    * lum
                    * gauss.pdf(mesh_list).reshape(grid_size, grid_size)
                )
            ...
            saveImage(
                os.path.join(dir_path, "{:04d}_{:7.3f}km.png".format(i, ht / 1000)),
                frame,
                cmap='gray',
                vmin=0,
                vmax=255,
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(
        description="Run meteor ablation modelling using the given trajectory file."
    )
    ...
    arg_parser.add_argument(
        ...
        '--load',
        metavar='LOAD_JSON',
        help="Load JSON file with fit parameters. Instead of giving the full path to the file, you can call it as '--load .' and it will automatically find the file if it exists.",
        type=str,
    )
    ...
    arg_parser.add_argument(
        ...
        '--met',
        metavar='MET_FILE',
        help='Load additional observations from a METAL or mirfit .met file.',
        type=str,
    )
    ...
    main_window = MetSimGUI(
        traj_pickle_file,
        const_json_file=load_file,
        met_path=met_file,
        lc_path=cml_args.lc,
        wid_files=wid_files,
    )
