...
if __name__ == '__main__':
    ...
    print()
    print("Filtered data:")
    print("Time (s)   Height (km)   Distance (km)   Velocity (km/s) Deceleration (km/s^2) DynPress (MPa)")
    for i in range(len(time_data)):
        if i == 0:
            decel = 0.0
        else:
            decel = (vel_s[i] - vel_s[i-1])/(time_data[i] - time_data[i-1])
        print("{:8.4f}   {:11.3f}   {:13.3f}   {:15.4f}   {:15.3f}   {:13.6f}".format(
            time_data[i], ht_data[i]/1000, dist_s[i]/1000, vel_s[i]/1000, decel/1000, dyn_pressure[i]/1e6))
