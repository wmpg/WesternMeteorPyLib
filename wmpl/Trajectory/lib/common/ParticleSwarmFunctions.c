...
FILE    *ParametersReadFile;
	...
	//======== Open the PSO parameters input file

    if( ( ParametersReadFile = fopen( PSOpathname, "r" ) ) == NULL )  {
        fprintf( stdout, "ERROR ===> PSO Config file %s could not be opened\n\n", PSOpathname );
        fprintf( stderr, "ERROR ===> PSO Config file %s could not be opened\n\n", PSOpathname );
        return(1);
    }
    ...
    if( read_version != 0.10 )  {
        fprintf( stdout, "ERROR ===> PSO Config file version %lf not valid for %s\n\n", read_version, PSOpathname );
        fprintf( stderr, "ERROR ===> PSO Config file version %lf not valid for %s\n\n", read_version, PSOpathname );
		return(1);
    }


	//======== Read the remainder of the configuration parameters

	fscanf( ParametersReadFile, "%[^=]= %ld", text, &psodata->nparticles      );
	fscanf( ParametersReadFile, "%[^=]= %ld", text, &psodata->maxiter         );
	fscanf( ParametersReadFile, "%[^=]= %ld", text, &psodata->boundary_flag   );
	fscanf( ParametersReadFile, "%[^=]= %ld", text, &psodata->limits_flag     );
	fscanf( ParametersReadFile, "%[^=]= %ld", text, &psodata->distro_flag     );

	fscanf( ParametersReadFile, "%[^=]= %lf", text, &psodata->winertia_init   );
	fscanf( ParametersReadFile, "%[^=]= %lf", text, &psodata->wstubborness    );
	fscanf( ParametersReadFile, "%[^=]= %lf", text, &psodata->wgrouppressure  );
	fscanf( ParametersReadFile, "%[^=]= %lf", text, &psodata->eps_convergence );
...
double  random1, random2, grandom1, grandom2;
	...
	for( kparam=0; kparam<pso->nparams; kparam++ )  {
	    ...
	    pso->vcurr[kparticle][kparam] = 0.001 * pso->vmaxx[kparam];
	...
	if( pso->distro_flag == PARTICLEDISTRO_RANDOM )  {
		...
		for( kparticle=1; kparticle<pso->nparticles; kparticle++ )  {
			...
			for( kparam=0; kparam<pso->nparams; kparam++ )  {
				...
				if( random1 > 0.5 )  pso->vcurr[kparticle][kparam] = +pso->vmaxx[kparam] * ( 1.0 - random2 );
				else                 pso->vcurr[kparticle][kparam] = -pso->vmaxx[kparam] * ( 1.0 - random2 );
...
double  random1, random2, random3, vel, xprev;
    ...
    for( kparam=0; kparam<pso->nparams; kparam++ )  {
		...
		if (fabs(vel) > pso->vmaxx[kparam]) {
			if( vel > 0.0 )  vel = +pso->vmaxx[kparam];  //... limit the speed, keep the sign
			else             vel = -pso->vmaxx[kparam];
		}
