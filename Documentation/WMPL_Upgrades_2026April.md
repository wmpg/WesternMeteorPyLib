...
Modes 1 and 2 can be run with a period of around 30 minutes (`--autofreq 30`), and should be restricted to at most 500 trajectories at a time, using `--maxtraj 500`. This ensures that candidates are processed in reasonable batches without requiring excessive memory. It also supports distributed processing as explained in the next section. 
